...
\begin{equation*}
P_A(s) = s^n + a_1s^{n-1}+a_2s^{n-2}+ \cdots + a_{n-1}s + a_n
\end{equation*}
se cumple que,
\begin{equation}\label{eq:ch}
P_A(A) = A^n + a_1A^{n-1}+a_2A^{n-2}+ \cdots + a_{n-1}A + a_nI  = 0_{n\times n}
\end{equation}
...
\begin{enumerate}
\setcounter{enumi}{4}
\item Para toda matriz $A$  de dimension $n\times n$, existen n funciones escalares, $\beta_0(t),\beta_1(t)\cdots,\beta_{n-1}(t)$ tales que,
...
\end{equation}
\end{enumerate}
Estas dos propiedades se puede deducir del teorema de Cayley-Hamilton. Despejando en (\ref{eq:ch}) $A^n$ en función de todos los demás términos,
$A^n = - a_1A^{n-1}-a_2A^{n-2}- \cdots - a_{n-1}A - a_n$. Si ahora multiplicamos a ambos lados por $A$ obtenemos $A^{n+1} = - a_1A^{n}-a_2A^{n-1}- \cdots - a_{n-1}A^2 - a_nA$, podríamos a continuación volver a sustituir $A^n$ en el primer término de este último polinomio por el valor obtenido a partir del teorema de Cayley-Hamilton, con lo que obtendríamos una expresión polinómica para $A^{n+1}$, en función de $A^{n-1},A^{n-2}, \cdots,I$. Es fácil ver, que repitiendo el mismo procedimiento, podemos expresar cualquier potencia de $A$ en función de sus primeras $n-1$ potencias,
\begin{equation}
A^k  = \alpha_{n-1}(k)A^{n-1}+\alpha_{n-2}(k)A^{n-2}+ \cdots + \alpha_{1}(k)A + \alpha_0(k)I 
\end{equation}
...
\begin{definition}[Multiplicidad algebraica de un autovalor]
Dado una matriz $A$ de dimensión $n\times n$, se define como multiplicidad algebraica de un autovalor $\lambda$ de la matriz $A$ al número de veces que dicho autovalor aparece como raíz del polinomio, $P_A(s)$, característico de $A$. 
\qed
\end{definition}
