...
\begin{enumerate}
	\item es \emph{(marginalmente) estable} si para cada condición inicial $x_0$, entonces $x(t) = \Phi(t,t_0) x_0$ está acotada uniformamente\footnote{Una señal $x:[0,\infty) \to \mathbb{R}^n$ es uniformemente acotada si existe una constante $C>0$ tal que $||x(t)|| \leq c, \forall t \geq 0$.} para todo $t>t_0$.
...
Comprobando las soluciones (\ref{eq: solx})-(\ref{eq: soly}), podemos decir que si $A$ tiene coeficientes constantes y $\dot x = Ax$ es asintóticamente estable, entonces $x(t) \to \int_{t_0}^t e^{A(t-\tau)}Bu(\tau)d\tau$ según $t\to\infty$. 
...
\begin{theorem}
	\label{thm: tayl}
	Asume que $f(x)$ es dos veces diferenciable. Si (\ref{eq: delta}) es exponencialmente estable, entonces, existe un entorno $\mathcal{B}$ alrededor de $x^*$ y constantes $c, \lambda > 0$ tal que para cada solución $x(t)$ del sistema (\ref{eq: non}) que empiece con $x(t_0)\in\mathcal{B}$, tenemos que
	\begin{equation}
...
El siguiente teorema solo se aplica cuando $A$ es una matriz de estabilidad.
\begin{theorem}\label{TLY} (Test de Lyapunov para la controlabilidad.) Si $A$ es una matriz de estabilidad, el par (constante) $(A,B)$ es controlable si y solo sí existe una única solución definida positiva $W$ para la siguiente ecuación de Lyapunov:
	\begin{equation}
		AW+WA^T = -BB^T
	\end{equation}  
\end{theorem}
...
\section{Estabilización basada en el test de Lyapunov de controlabilidad. Teorema \ref{TLY}}

Si el par $(A,B)$ es controlable, entonces el par $(\mu I-A,B)$ también lo es. Esto puede probarse empleando el test de autovectores, ya que $A^T$ y $-\mu I-A^T$ comparten los mismos autovalores,
\begin{equation*}
	A^Tx = \lambda x \iff (-\mu - A^T)x = (-\mu -\lambda )x
\end{equation*}

Mediante una elección adecuada de $\mu$, siempre podemos hacer que $(-\mu - A^T)$ sea una matriz de estabilidad y, por tanto, que $(-\mu -A)$ también lo sea.

Supongamos que efectivamente elegimos $\mu$ suficientemente grande de modo  que $(-\mu -A)$ es una matriz de estabilidad. Entonces, por el test de Lyapunov sabemos que debe existir una matriz $W$ que satisface la ecuación
\begin{equation}
	(-\mu I - A)W + W(-\mu I -A)^T = BB^T \Rightarrow AW + WA^T - BB^T =-2\mu W
\end{equation}

Si multiplicamos a ambos lados por $P := W^{-1}$.

\begin{equation*}
	PA +A^TP -PBB^TP = -2\mu P,
\end{equation*}
y, definiendo $K =\frac{1}{2}B^TP$, podemos reescribirla como,

\begin{equation}
	P(A-BK)+(A-BK)^TP = -2\mu P
\end{equation}

Pero esta ecuación es precisamente la del test de estabilidad de Lyapunov. Que se satisface si $\mu >0$, ya que $p \succ 0$.

Si elegimos una entrada para nuestro sistema de la forma $-Kx$, 

\begin{equation*}
\dot x = Ax - bKx \rightarrow \dot x = (A - BK)x
\end{equation*}

Se estabiliza asintóticamente. Se dice que la entrada se \emph{realimenta} de los estados del sistema, y el procedimiento se conoce como estabilización por realimentación de estados. 
...
\end{theorem}
\begin{proof}
	Solo vamos a ver una dirección de la prueba. No confundir (\ref{eq: lb}) con la ecuación de Lyapunov\footnote{Observa el orden de las matrices traspuestas, y observa también el signo de $-BB^T$ si comparas las dos ecuaciones.} $PA+A^TP \prec 0$ en (\ref{eq: lya}).
