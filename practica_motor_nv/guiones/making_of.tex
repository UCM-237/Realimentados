...
Aquí termina el contenido del bucle y la función \texttt{main}. El resto del código que contiene el archivo, son funciones de inicialización de periféricos y configuración de la placa y han sido generadas directamente por STM32CubeMX.
...
A continuación definimos la estructura \texttt{motor} que declaramos en \texttt{motor.h}, y declaramos conmo \texttt{extern} las dos estructuras \texttt{htim2} y \texttt{htim3} que definimos en \texttt{main.c}, para manejar los timers que controlan la lectura de encoders y la generación por hardware de las señales de PWM necesarias para dar tensión al motor.
...
A continuación, empieza la definición de las funciones de control del motor. La primera es una función de de inicialización que se llama al principio de la función \texttt{main()} del archivo \texttt{main.c}. Inicializa los dos modos posibles de control, dejandolos listos para su uso.
...
\item Control de posición usando variables de estado y un estimador. Las matrices necesarias ya se definieron como variables globales antes
\item Control de velocidad usando variables de estado y un estimador. Igual que el ne caso anterior las matrices ya están definidas
\end{enumerate}
