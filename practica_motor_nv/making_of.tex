...
Una de las características interesantes de la \emph{Black Pill} es su velocidad. Vamos a configurar el reloj de la placa para que trabajea 10MHz, que es la máxima velocidad disponible.

Para ello, volvemos a la categoría \texttt{System Core} y seleccionamos la entrada \texttt{RCC} (Reset and Clock Control). Buscamos la opción High Speed Clock (\textttt{HSE}). Cambiamos  "Disable" por \texttt{BYPASS Clock Source}. Hacemos esta elección porque en la placa, la señal de reloj la toma el microcontrolador directamente del programador (ST-Link). Los pines PH0 y PH1 aparecen ahora de color verde.
